...
class Solution:
    ...
    # put first values to the track for solution 2 -> the longest route
    # similar to minimum route, BUT the track value will be stored as negative to keep the largest always first
    def prepare_queue_2(self) -> None:
        self.track_queue_2 = PriorityQueue()
        self.visited_configs_2 = {i: [] for i in range(3, len(self.all_cities) + 1)}
        # for all combinations of cities, put them in the queue
        for city1, city2 in combinations(self.all_cities, 2):
            # put in queue : len of track, actual track value, track
            self.track_queue_2.put((2, -1 * self.graph[city1][city2], [city1, city2]))
            self.track_queue_2.put((2, -1 * self.graph[city1][city2], [city2, city1]))
        # number of all cities is max_track_lenght
        self.max_track_len = len(self.all_cities)
    ...
    def solution_2(self):
        # prepare the queue
        self.prepare_queue_2()
        # while queue is not empty
        while not self.track_queue_2.empty():
            act_len, act_track_val, act_track = self.track_queue_2.get()
            # if track is of maximum lenght, check if it is the shortest one
            # as it is a priority queue, first one should be the smallest one
            if act_len == self.max_track_len:
                return -1 * act_track_val
            # expand track always to the right
            for new_city in self.all_cities.difference(set(act_track)):
                new_track = act_track + [new_city]
                if new_track not in self.visited_configs_2[act_len + 1]: 
                    self.visited_configs_2[act_len + 1].append(new_track)
                    self.track_queue_2.put((act_len + 1, act_track_val - self.graph[act_track[-1]][new_city], new_track))
