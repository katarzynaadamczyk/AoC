   ...
   # for y in range(len(data)):
    while y < len(data):
        ...
        if y <= 0:
            ...
            if tmp < result[y][-1] and not resultb[y][-1]:
                result[y][-1] = tmp
                changesmade += 1
                resultb[y][-1] = True
              #  print(y, -1)
            for x in range(len(data[y]) - 2, 0, -1):
                ...
                if tmp < result[y][x] and not resultb[y][x]:
                    result[y][x] = tmp
                    changesmade += 1
                    resultb[y][x] = True
                #    print(y, x)
            ...
            if tmp < result[y][0] and not resultb[y][0]:
                result[y][-1] = tmp
                resultb[y][0] = True
                #changesmade += 1
              #  print(y, 0)
            ...
            if result[-1][-1] > tmp and not resultb[-1][-1]:
                result[-1][-1] = tmp
                resultb[-1][-1] = True
                break
        else:
