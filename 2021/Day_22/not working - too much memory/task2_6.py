...
def removeduplicates(cubeson):
    toremove = []
    for i in range(len(cubeson) - 1):
        for j in range(i + 1, len(cubeson)):
            if checkifincube(cubeson[j], cubeson[i]):
                toremove.append(j)
            elif checkifincube(cubeson[i], cubeson[j]):
                toremove.append(i)
    ...
    while changesmade:
        ...
        for i in range(len(cubeson) - 1):
            ...
            if changesmade:
                break
        if changesmade:
            newitems += dividecube(cubeson[j], cubeson[i])
            cubeson.pop(j)
    print(newitems)
    #cubeson += newitems
...
def addlights(newcube, cubeson):
    if len(cubeson) < 1:
        ...
        cubeson = removeduplicates(cubeson)
        cubeson = removeoverlapping(cubeson)
...
def removelights(cubetodarken, cubeson):
    if len(cubeson) >= 1:
        cubestodarken = [cubetodarken]
        lst = []
        newcubestodarken = []
        for cubetodivide in cubeson:
            
            newcubestodarken = []
            for cubetodarken in cubestodarken:
                if not checkifincube(cubetodivide, cubetodarken):
                    lst += dividecube(cubetodivide, cubetodarken, False)
                    newcubestodarken += dividecube(cubetodarken, cubetodivide)
            cubestodarken = newcubestodarken
        cubeson = lst
    return cubeson
...
def main():  
    ...
    print(f'Result for test data for task 1 is {solution1("Day_22/testdata3001.txt")} (should be 590784)')
