...
class Solution:
    ...
    def __init__(self, filename) -> None:
        ...
        self.num_of_sides = {'up': self.get_number_of_up_down_sides, 
    ...
    def get_up_down_left_right_sides(self, sides, garden):
    ...
    def get_number_of_left_right_sides(self, sides):
        num = 0
        for x in set([a[1] for a in sides]):
            y = sorted([a[0] for a in sides if a[1] == x])
            if len(y) == 1:
                num += 1
                continue
    ...
    def get_number_of_up_down_sides(self, sides):
        num = 0
        for y in set([a[0] for a in sides]):
            x = sorted([a[1] for a in sides if a[0] == y])
            if len(x) == 1:
                num += 1
                continue
            diffs = [x2 - x1 for x1, x2 in zip(x[:-1], x[1:])]
            num += 1 + len([a for a in diffs if a != 1])
        return num
    ...
    def get_garden_sides(self, garden):
        ...
        for point in garden:
            for dir in self.dir_dict.values():
                ...
                if new_point not in garden:
                    sides.add(point)
        sides = self.get_up_down_left_right_sides(sides, garden)
        result = 0
        for direction, points in sides.items():
            result += self.num_of_sides[direction](points)
